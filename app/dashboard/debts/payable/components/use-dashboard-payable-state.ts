...
import { getPayables, type PayableResponseDTO } from "@/api/payables_api"
...
export function useDashboardPayableState(locale: LandingLocale) {
  const [records, setRecords] = useState<PayableResponseDTO[]>([])
  ...
  return {
    ...
    pageSize,
    pageStartIndex,
    prependRecord: (record: PayableResponseDTO) =>
